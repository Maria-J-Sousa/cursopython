...
# Configurações da tela (aumentada)
...
palavras_dicas = {
    "amazônia": ["Maior floresta tropical do mundo", "Conhecida como 'pulmão do mundo'", "Ocupa cerca de 40% do território brasileiro"],
    "carnaval": ["Maior festa popular do país", "Tem origem no entrudo português", "No Rio de Janeiro, dura quase uma semana"],
    "futebol": ["Esporte mais popular no Brasil", "O país tem 5 títulos mundiais", "Pelé é considerado o maior jogador da história"],
    "café": ["Brasil é o maior produtor mundial", "Foi a base da economia no século XIX", "São Paulo cresceu graças a esta cultura"],
    "samba": ["Gênero musical brasileiro", "Nasceu nas comunidades afro-brasileiras", "Estilo tocado nas escolas de samba"],
    "capoeira": ["Arte marcial afro-brasileira", "Combina dança, música e acrobacias", "Foi desenvolvida por escravos no período colonial"],
    "bossa nova": ["Estilo musical brasileiro", "Ficou famoso mundialmente na década de 1960", "Tom Jobim e João Gilberto são seus maiores expoentes"],
    "açaí": ["Fruto típico da região norte", "Tornou-se popular em todo o país", "Rico em antioxidantes e energia"],
    "pão de açúcar": ["Morro no Rio de Janeiro", "Um dos cartões postais mais famosos", "Nome vem de um tipo de embalagem de açúcar no século XVI"],
    "ipê": ["Árvore símbolo do Brasil", "Floresce em diferentes cores", "Cada cor representa uma região do país"],
    "cuiabá": ["Capital do Mato Grosso", "Conhecida como 'cidade verde'", "Porta de entrada para o Pantanal"],
    "pelé": ["Maior jogador de futebol de todos os tempos", "Apelidado de 'Rei do Futebol'", "Único jogador tricampeão mundial"],
    "tietê": ["Rio mais importante de São Paulo", "Atravessa a capital paulista", "Um dos mais poluídos do país"],
    "bumba meu boi": ["Festividade cultural brasileira", "Mistura dança, música e teatro", "Típica da região nordeste"],
    "cachaça": ["Bebida destilada brasileira", "Feita a partir da cana-de-açúcar", "Ingrediente principal da caipirinha"],
    "fernando de noronha": ["Arquipélago no nordeste brasileiro", "Patrimônio Natural da Humanidade", "Um dos melhores locais para mergulho no país"],
    "cristo redentor": ["Estátua no Rio de Janeiro", "Uma das 7 maravilhas do mundo moderno", "Símbolo do cristianismo brasileiro"],
    "inconfidência mineira": ["Movimento pela independência do Brasil", "Ocorreu em Minas Gerais no século XVIII", "Tiradentes foi seu mártir mais famoso"],
    "literatura de cordel": ["Tipo de poesia popular brasileira", "Originária do nordeste", "Exposta em folhetos pendurados em cordas"],
    "feijoada": ["Prato típico brasileiro", "Considerada o prato nacional", "Surgiu nas senzalas com restos de carne"]
}

class Placar:
    def __init__(self):
        self.pontos = 0.0
        self.anim_offset = [0] * 20
        self.anim_time = 0
        self.scale = [1.0] * 20
    
    def adicionar_vitoria(self):
        self.pontos += 1.0
        # Efeito especial ao ganhar pontos
    ...
    def atualizar_animacao(self):
        ...
        for i in range(len(self.anim_offset)):
            # Efeito de onda mais suave
            self.anim_offset[i] = math.sin(self.anim_time + i * 0.3) * 4
            
            # Reduz gradualmente o efeito de scale
            if self.scale[i] > 1.0:
                self.scale[i] -= 0.05
    ...
    def mostrar(self, y):
        ...
        # Fundo do placar com as cores do Brasil
        ...
        # Renderiza cada caractere com efeitos
        for i, char in enumerate(placar_text):
            # Alterna entre azul e amarelo (cores da bandeira)
            color = BRASIL_BLUE if i % 2 == 0 else BRASIL_YELLOW
            
            # Calcula posição com animação
            char_width = font_placar.size(char)[0]
            pos_x = x + sum(font_placar.size(placar_text[:k])[0] for k in range(i))
            
            # Cria superfície escalada
            char_surface = font_placar.render(char, True, color)
            scaled_width = int(char_surface.get_width() * self.scale[i])
            scaled_height = int(char_surface.get_height() * self.scale[i])
            scaled_surface = pygame.transform.scale(char_surface, (scaled_width, scaled_height))
            
            # Posiciona o caractere
            screen.blit(scaled_surface, 
                       (pos_x + (char_surface.get_width() - scaled_width) // 2, 
                        y + self.anim_offset[i] + (char_surface.get_height() - scaled_height) // 2))
...
def tela_inicial(placar):
    while True:
        ...
        # Fundo temático
        pygame.draw.rect(screen, (0, 50, 0), (0, 0, WIDTH, HEIGHT//3))  # Verde
        pygame.draw.rect(screen, (200, 150, 0), (0, HEIGHT//3, WIDTH, HEIGHT//3))  # Amarelo
        pygame.draw.rect(screen, (0, 30, 100), (0, 2*HEIGHT//3, WIDTH, HEIGHT//3))  # Azul
...
def jogo(placar):
    ...
    while tentativas > 0 and not acertou:
        ...
        mostrar_texto(f"Dica {dica_idx + 1}: {dicas[dica_idx]}", font_medium, WHITE, 170)
